...
export function generarPDF(name: string, item: {
    id: number;
    initial_amount: number;
    used_batches: { amount_used_to_produce_batch: number; batch_code: string, ingredient_name: string, ingredient_unit: string }[];
}) {
    ...
    doc.text(
        'Cantidad a producir:' + item.initial_amount + ' de ' + name + '.',
        10,
        23
    );
    ...
    autoTable(doc, {
        ...
        body: item.used_batches.map((x) => {
            return [x.ingredient_name, x.amount_used_to_produce_batch.toString() + ' ' + x.ingredient_unit, x.batch_code];
        })
    });
