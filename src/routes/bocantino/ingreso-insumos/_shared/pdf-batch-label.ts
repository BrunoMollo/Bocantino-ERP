...
function generateBatchLabel(data: BatchLabelData, filename: string) {
	...
	doc.text('Vencimiento: ' + expiration_date, pageWidth - margin, dateYPosition, {
		align: 'right'
	});

	doc.setFontSize(7);
	doc.text(
		'© ' + this_year + ' BOCANTINO. Todos los derechos reservados.',
		margin,
		pageHeight - margin
	);
...
export function printIngredientBatchLabel(batch: {
	id: number;
	batch_code: string;
	initial_amount: number;
...
export function printBatchLabelFromDetail(batch: {
	id: number;
	code: string;
	ingredient: string;
	initial_amount: number;
