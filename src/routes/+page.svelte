		...
		{#each listaTipos as tipoMateriaPrima}
			<div
				...
				style="background-color:{tipoMateriaPrima.stockR / tipoMateriaPrima.stockD > 0.5
					...
					tipoMateriaPrima.stockD >
				0.5
					? ''
					: '0 2px 25px 2px rgba(255, 0, 0, 0.8)'}"
			>
				{#if tipoMateriaPrima.stockR / tipoMateriaPrima.stockD < 0.5}
					...
					<div class="absolute right-5 bottom-4">
						<ProgressRadial
							...
							font={128}
							stroke={70}
							value={(tipoMateriaPrima.stockR / tipoMateriaPrima.stockD) * 100}
							>{Math.round(
								(tipoMateriaPrima.stockR / tipoMateriaPrima.stockD) * 100
							)}%</ProgressRadial
						>
					</div>
				{/if}
